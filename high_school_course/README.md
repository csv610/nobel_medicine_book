# Medicine Discovery: A High-School Course

This course is designed as a mentored exploration of 232 important medical investigations. Students can choose topics that match their interests without losing the guidance of a clear learning path.

## Audience and format

- **Audience:** students approximately 14–18 years old; no advanced biology required.
- **Structure:** 232 short, selectable scientist chapters rather than a fixed sequence.
- **Method:** read the chapter, review the foundation material, complete the safe project, and reflect on evidence and ethics.
- **Materials:** notebook, graph paper or spreadsheet, ruler, household objects, and optionally a phone camera or free spreadsheet software.

The projects are designed for classrooms, clubs, or independent study. They do not require culturing microorganisms, human-subject experiments, animal work, medical treatment, or handling biological samples.

## Start here

### Students

1. Read [foundations](foundations.md).
2. Read [safety and ethics](safety_ethics.md).
3. Use the [assessment guide](assessment.md) for any selected chapter.
4. Browse the [subfield groups](subfields/) or the complete [232 scientist chapters](chapters/).

### Mentors

Use the separate [mentor guide](mentor_guide.md) to introduce subfields, support inquiry, and reveal historical recognition only after students have explored the evidence.

Every chapter begins with a “Why this matters” curiosity hook and a mentor question. It then introduces the scientist and research focus, gives a link to extended background reading, and provides a safe project, report requirements, and reflection questions. Recognition history is reserved for the mentor's later debrief.

## Choosing a path

Students may select chapters by person, topic, subfield, or project type. Teachers can assign a common theme while allowing each student to choose a scientist within that theme. The course uses eight browsing groups:

- [Infection, Immunity, and Vaccines](subfields/infection_immunity.md)
- [Genetics, Genomics, and Development](subfields/genetics_development.md)
- [Neuroscience, Senses, and Behavior](subfields/neuroscience_behavior.md)
- [Physiology, Metabolism, and Endocrinology](subfields/physiology_metabolism.md)
- [Cell Biology and Molecular Systems](subfields/cell_molecular.md)
- [Therapeutics, Surgery, and Public Health](subfields/therapeutics_public_health.md)
- [Reproduction, Aging, and Human Origins](subfields/reproduction_origins.md)
- [Medical Technology and Research Methods](subfields/technology_methods.md)

## Source policy

Students should distinguish between an original observation, later evidence, and a modern application. The mentor may reveal historical recognition after students have developed and tested their own ideas; recognition is evidence of historical impact, not proof that every modern claim is correct.
